...
Partition non-amplification, also called subdivision monotonicity (P6), is a separate obligation, stated in Section 1.2 over a fixed input workload with releases derived: subdividing one root's demand across AttributionChildScopes beneath it must, at every decision prefix, give that root no greater cumulative selections and no greater cumulative admitted quantity than the unsubdivided baseline, and must delay no competitor's baseline selection. The installed provider's scheduling is root-keyed and a Construction A control covers it; see Sections 5.2 and 7.1 for what that control does and does not establish.
...
**Production currently mints exactly one process root.** Additional trusted-root minting is `#[cfg(test)]` and crate-local, and the cross-root controls exercise it only under test. **This report therefore claims no deployed multi-root mapping.** What the change establishes for production is narrower and concrete: subdividing a root's demand no longer creates extra scheduling turns, because the disclosed rotation-key counterexample is removed. That is not a structural guarantee of P6 in general — see the bounded-evidence limits in Section 7.1. No deployment-level mapping from local subjects to distinct roots exists yet, and none may be reported as shipped.
...
**Control status.** The P6 non-amplification obligation now has an implementing control. A Construction A control runs a baseline and a subdivided mapping against the live provider and evaluates the prefix comparison, and a negative oracle control drives the same harness with scope-keyed selection forced on and requires the oracle to reject it — so the oracle is shown to be capable of failing rather than merely passing. Both pass on the working tree.

Three limits on that result, all load-bearing:

- **It is a bounded control, not a universal proof.** What the implementation removes is the concrete per-scope amplification mechanism; what the control establishes is that a specific Construction A workload does not amplify, with a non-vacuous negative fixture proving the oracle can fail. Neither is a mechanized proof over every possible P6 workload. The canonical property in Section 1.2 remains the obligation; this is one discharged instance of it, and it must not be promoted into a whole-model proof.
- **It is working-tree evidence only.** These results are not exact-head evidence and not hosted CI. Until they run on a committed and pushed head, nothing here may be cited as an exact-head or CI result, and the distinction is the same one applied to `7e2ba9e` and `6a22911`.
- **The control's `DemandId`s are test-side.** Construction A uses logical `DemandId`s mapped positionally from the provider's selection log. They are not caller-supplied provider identifiers, and the provider exposes no such identifier. The control is therefore a conformance result about selection order under subdivision, not evidence that any public API carries demand identity.

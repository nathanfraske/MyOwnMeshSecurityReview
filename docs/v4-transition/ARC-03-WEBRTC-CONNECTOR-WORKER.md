...
- service weight is not multipliable. Scheduling attributes service to a `FairnessRoot`, and creating, cloning, or rotating any number of attribution child scopes beneath one claimant's `FairnessRoot` cannot improve that claimant's service share against another `FairnessRoot`. Both terms are used with the closed definitions in [`IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md`](../../IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md): a `FairnessRoot` is the trusted local attribution a provider schedules against and is not mintable by the claimant it attributes; an attribution child scope refines accounting beneath exactly one `FairnessRoot` and creates no additional root, share, or turn. A connector-local service weight or quantum is separate connector-local ordering and does not discharge this property.
...
Disclosed nonconformance. The shipped `FiniteResourceProvider` does not satisfy the non-multipliable service-weight property. Its equal-class rotation is keyed to process-local scope identities, which a claimant can create by creating attribution child scopes, so the provider schedules against a mintable scope identity rather than against a `FairnessRoot`, and a claimant holding several scopes collects several turns against a claimant holding one. No Arc 03 control proves that property, and the existing rotation and yield controls exercise the mintable mechanism itself, so none of them may be cited as evidence for it. The property is unchanged and remains basal; the shortfall is in the provider. The correction belongs to the named provider fairness slice, which must bind a pending demand to a `FairnessRoot` the claimant cannot create, split, name, rotate, or multiply, and must supply the missing control. Until that control exists and passes, this obligation is reported as open and failing. Arc 03 claims no pass for it here.

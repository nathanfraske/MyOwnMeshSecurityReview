...
- work conservation constrains immediate refusal. An immediate, nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit means `EffectiveFit`. `AccountingFit` is the remaining committed accounting grant after the live and failed-cleanup-retained charge `S`, and after explicit P5 isolation or optional local policy. `EffectiveFit` is `AccountingFit` intersected with `E` only where `E` is proved and with `B` only where `B` is proved, independently and per dimension. Where neither is proved, `EffectiveFit` equals `AccountingFit` — which is the shipped provider's case, since it proves neither. Neither definition takes `O` or `T` as an input, and no provider may intersect against an `E` or `B` it does not actually have. **A successful admission does not guarantee allocator, kernel, runtime, transport, external-relay, or hardware success**; those failures remain real and application-visible, and connector callers must still handle them. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not be used to hold capacity for an anticipated demand, to smooth a demand rate, or to enforce an undeclared share, because any such reservation is a partition and is conforming only as an explicit local isolation policy.
- contraction is modeled over six distinct quantities per dimension, and no two may be conflated: `S` the committed charge, `Gc` the provider-owned committed grant that admission is checked against, `O` an optional and inert observation or measurement, `T` an explicit owner-selected contraction target, `E` an enforceable isolation envelope or ceiling, and `B` an actual reserved or owned guarantee. **`E` and `B` are orthogonal premises, not a hierarchy.** A cgroup, job object, process limit, or appliance bound is an `E`: it stops the process at that value. That is containment, and it reserves nothing — a process inside an envelope has no guarantee the capacity is obtainable, only that it will be stopped. `B` is capacity actually held for this process. Neither implies the other: containment without reservation is ordinary, and reservation without containment is equally possible. Each is proved separately and **per resource dimension**, so a connector deployment may hold `E` in one dimension, `B` in another, both, or neither, and may claim only what is proved for that dimension. `S <= Gc` holds in every dimension at every moment, so a grant may be contracted only down to the charges already committed. **`O` is optional and inert**: it changes no grant, admission result, or contraction state, sets nothing automatically, need not exist, and creates no provider class. **`T` arises only by named owner policy**, either set directly or derived by a named policy that considers `O`; `T < Gc` requests gradual contraction, and `Gc` follows downward only after owner-driven release lowers `S`, never below `S`. A provider proves `Gc <= E` only where an envelope actually exists and `Gc <= B` only where capacity is actually reserved. `E < Gc` or `E < S` is a typed envelope shortfall and `B < Gc` or `B < S` a typed backing loss, reported distinctly: the provider retains every charge, refuses new work conflicting with the shortfall, issues only exact retirement requests to owners whose lease contracts declare those leases reclaimable, and **claims no envelope or backing that is not there**. The shortfall is never available capacity and never becomes reclamation authority. Where backing cannot be proved at all, **that is the open Slice C residual**: the dimension is an unproved-backing residual, the provider has not thereby failed, and it simply may not assert it. Arc 03 does not close the Slice C question and claims no enforceable envelope and no reserved guarantee for any dimension.
...
The determinism of that policy is narrow, and it is a positive claim with an exact scope. Given identical already-issued `ResourceScopeId` values, identical provider state, and the same ordered sequence of provider operations, arbitration is deterministic: the same demand is selected and the same result is produced. Nothing beyond that is claimed. `ResourceScopeId` is the allocation address of the scope identity, so which identities are issued, and therefore the equal-authority rotation order, is not reproducible across runs, processes, allocators, builds, platforms, or thread schedules, and an address may be reused once its scope identity drops. A scope identity is unique only among live scopes, never over the life of the process. No Arc 03 caller, control, or diagnostic may treat it as stable, meaningfully ordered, comparable across time, or unique over time, and no Arc 03 behavior may depend on a particular equal-class rotation sequence.

Disclosed nonconformance with P6, partition non-amplification or subdivision monotonicity. The shipped `FiniteResourceProvider` does not satisfy it. Its equal-class rotation is keyed to process-local scope identities, so several `AttributionChildScope` values beneath one fixed `FairnessRoot` act as several distinct rotation keys. Running one fixed input workload with that root's demand subdivided across those scopes obtains extra rotation turns: the root's cumulative selections exceed the unsubdivided baseline at some decision prefix, and a competitor's baseline selection is pushed later. That breaks the prefix inequalities the bound requires. No Arc 03 control runs both the baseline and subdivided workloads and evaluates the prefix inequalities per dimension and per competitor, and the existing rotation and yield controls exercise the per-scope mechanism at issue, so none of them may be cited as evidence for it. In particular, no control yet exists in the first required form — topology-normalized over identical pre-existing scopes and identical bookkeeping claims with stable `DemandId`s, driven by the deterministic clock-free environment with terminal stuttering, over a bounded decision prefix that starts identical in both runs and during which none of the compared newly admitted work releases. That release condition is narrower than prohibiting all releases: unrelated releases elsewhere in the workload are permitted, and only a compared demand releasing inside the prefix would confound the comparison. Because no such control exists, subdivision has never been compared against an unsubdivided baseline under Arc 03 at all. The property is unchanged and remains basal; the shortfall is in the provider. The correction belongs to the named provider fairness slice, which must bind each pending demand to the fixed `FairnessRoot` that owns it rather than to a per-scope identity, and must supply the missing control. Until that control exists and passes, this obligation is reported as open and failing. Arc 03 claims no pass for it here.

...
Review `4869373979` at `f58dab6` is cited as provenance for the fairness wording and harness corrections recorded below. Review `4870701740` is cited as provenance for the P6 restatement as partition non-amplification over a fixed input workload and for the prefix-inequality control form. Controlling owner review `4870850580`, anchored at exact head `7e2ba9e5ee042d3aa6c39f16670ab9a646b44e41`, is cited as provenance for the topology-normalized P6 comparison, the `E` and `B` split with declared provider classes, and the liveness bound on typed reporting. A review identifier is provenance for a change, never execution proof. No obligation in this document is discharged by citing it.
...
- work conservation constrains immediate refusal. An immediate, nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit is evaluated against `Gc` net of `S` narrowed by any explicit local isolation policy or optional ceiling, then additionally bounded by `E` where `E` is proved and by `B` where `B` is proved, independently and per dimension. The shipped provider proves neither and has neither bound to apply. Fit is never evaluated against `O`, and no provider may bound against an `E` or `B` it does not actually have. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not be used to hold capacity for an anticipated demand, to smooth a demand rate, or to enforce an undeclared share, because any such reservation is a partition and is conforming only as an explicit local isolation policy.

...
Review `4869373979` at `f58dab6` is cited as provenance for the fairness wording and harness corrections recorded below. A review identifier is provenance for a change, never execution proof. No obligation in this document is discharged by citing it.
...
- P6 finite-trace partition invariance, in its one-way non-amplification form: scheduling is non-amplifying under repartition, on any finite trace of a fixed set of `FairnessRoot` values. Fix the roots and a finite trace of demands, grants, releases, and refusals, take one fixed root's identical demand trace, and repartition it across any number of `AttributionChildScope` values beneath that same root while changing nothing else. Against the unpartitioned trace, the repartitioned trace must not make that root eligible earlier, must not give it more selections or turns, must not increase the quantity admitted to it, and must not delay a competing fixed root. The bound is over those four observables; it does not require the whole service outcome to be equal and does not claim that partitioning is undetectable. Both terms are used with the closed definitions in [`IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md`](../../IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md): a `FairnessRoot` is the trusted local attribution a provider schedules against and is not mintable by the claimant it attributes; an `AttributionChildScope` refines accounting beneath exactly one `FairnessRoot`. A connector-local service weight or quantum is separate connector-local ordering and does not discharge P6.
...
- committed charges never exceed the provider-owned committed grant, in every dimension and at every moment, so a grant may be contracted only down to the charges already committed. Contraction applies only to later admission decisions, leaves every committed lease owned and charged, and releases, revokes, invalidates, and reuses nothing. External observed capacity is separate: the environment may report or impose less than the committed charges, and the provider then records a typed degraded or over-committed result relative to that observation while continuing to hold every charge. It does not lower the committed grant below the charges, does not forge, infer, or force a release, does not admit new work that conflicts with the shortfall, and issues only exact retirement requests to owners whose lease contracts declare those leases reclaimable. The shortfall is never available capacity and never becomes reclamation authority. The shipped provider fixes its grant at construction, exposes no contraction API, and models no external observed capacity, and no Arc 03 control exercises either.
...
Disclosed nonconformance with P6, the finite-trace partition property (the four-observable non-amplification bound). The shipped `FiniteResourceProvider` does not satisfy it. Its equal-class rotation is keyed to process-local scope identities, so several `AttributionChildScope` values beneath one fixed `FairnessRoot` act as several distinct rotation keys. Replaying one fixed root's identical demand trace repartitioned across those scopes obtains extra rotation turns: the root receives more selections than the unpartitioned trace gives it, and the competing fixed root is delayed. That is exactly the amplification the bound forbids. No Arc 03 control exhibits both traces and compares the four observables, and the existing rotation and yield controls exercise the per-scope mechanism at issue, so none of them may be cited as evidence for it. The property is unchanged and remains basal; the shortfall is in the provider. The correction belongs to the named provider fairness slice, which must bind each pending demand to the fixed `FairnessRoot` that owns it rather than to a per-scope identity, and must supply the missing control. Until that control exists and passes, this obligation is reported as open and failing. Arc 03 claims no pass for it here.

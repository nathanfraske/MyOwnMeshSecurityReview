...
Review `4869373979` at `f58dab6` is cited as provenance for the fairness wording and harness corrections recorded below. Review `4870701740` is cited as provenance for the P6 restatement as partition non-amplification over a fixed input workload, for the prefix-inequality control form, and for the `S`/`Gc`/`O`/`T`/`B` contraction model. A review identifier is provenance for a change, never execution proof. No obligation in this document is discharged by citing it.
...
- P6 partition non-amplification, also called subdivision monotonicity. What is held fixed is the **input workload**, not a trace of outcomes: a finite set of `FairnessRoot` values, the initial provider state including the committed grant in every dimension, the arrival events, each demand's exact claim, authority class, and reclaimability, and one deterministic owner response rule for admitted work. Releases are **derived**, never fixed, because when an owner releases depends on when its work was admitted; fixing releases would compare two different workloads. Run the workload once with one fixed root `A` unsubdivided as baseline, then again with `A`'s demand spread across additional `AttributionChildScope` values beneath that same root and nothing else changed. The bound is a set of **prefix inequalities**, evaluated at every decision prefix `k`: `A`'s cumulative selections must not exceed baseline; `A`'s cumulative admitted quantity must not exceed baseline **in every resource dimension `d`**; and for **every competitor `B != A`**, each baseline selection of `B` must occur at a decision index no later than its baseline index, where a selection that never occurs takes index infinity. The bound is one-way: it does not require the whole service outcome to be equal, does not forbid `A` faring worse or a competitor faring better, and does not claim that subdivision is undetectable. Both terms are used with the closed architectural definitions in [`ARCHITECTURE.md`](../../ARCHITECTURE.md), which owns them: a `FairnessRoot` is the unit of scheduling attribution a provider serves, selected locally by the trusted provider or ingress owner that installs the grant, process-local and opaque, and not mintable by the claimant it attributes; an `AttributionChildScope` refines accounting beneath exactly one `FairnessRoot` and creates no additional share, turn, or service weight. This connector-facing summary neither redefines nor supersedes those terms; where it appears to differ, `ARCHITECTURE.md` governs. The exact P6 model and its comparison remain governed by [`FORMAL-PROOFS.md`](../../FORMAL-PROOFS.md) Note 14.5e, which fixes the causally closed run model and the prefix comparison; the statement here is the connector-facing summary of that note, not an independent formulation. A connector-local service weight or quantum is separate connector-local ordering and does not discharge P6.
- work conservation constrains immediate refusal. An immediate, nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit is evaluated against the committed and actually backed admission domain with P5 policy applied — `Gc` net of `S`, additionally bounded by `B` wherever substrate backing is claimed, and further narrowed by any explicit local isolation policy or optional ceiling. It is never evaluated against the committed grant alone, and never against a transient `O`. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not be used to hold capacity for an anticipated demand, to smooth a demand rate, or to enforce an undeclared share, because any such reservation is a partition and is conforming only as an explicit local isolation policy.
- contraction is modeled over five distinct quantities per dimension, and no two may be conflated: `S` the committed charge, `Gc` the provider-owned committed grant that admission is checked against, `O` a non-authoritative observation or measurement, `T` an explicit owner-selected contraction target, and `B` enforceable substrate backing or an assigned hard domain such as a cgroup, job object, process limit, appliance bound, or provider allocation. `S <= Gc` holds in every dimension at every moment, so a grant may be contracted only down to the charges already committed. **`O` alone changes nothing** — no grant, no admission result, no contraction state — and only a named, recorded policy action may convert an observation into a new `T`. **`T < Gc` requests gradual contraction**, and `Gc` follows `T` downward only after owner-driven release lowers `S`, never below `S`. **A provider claiming substrate-backed enforcement must prove `Gc <= B` at admission**; absent that proof it may not assert substrate backing. `B < Gc` or `B < S` is a typed backing-loss or external-overcommit state: the provider retains every charge, refuses new work conflicting with the shortfall, issues only exact retirement requests to owners whose lease contracts declare those leases reclaimable, and **does not pretend the missing physical backing exists**. The shortfall is never available capacity and never becomes reclamation authority. Where backing cannot be proved at all, **that is the open Slice C residual**: the dimension is an unproved-backing residual, the provider has not thereby failed, and it simply may not assert substrate-backed enforcement for that dimension. Arc 03 does not close the Slice C question and claims no enforceable backing for any dimension. The shipped provider fixes its grant at construction, exposes no contraction API, and models no `O`, `T`, or `B`, and no Arc 03 control exercises any of them — so every dimension is an unproved-backing residual at this head.
...
Disclosed nonconformance with P6, partition non-amplification or subdivision monotonicity. The shipped `FiniteResourceProvider` does not satisfy it. Its equal-class rotation is keyed to process-local scope identities, so several `AttributionChildScope` values beneath one fixed `FairnessRoot` act as several distinct rotation keys. Running one fixed input workload with that root's demand subdivided across those scopes obtains extra rotation turns: the root's cumulative selections exceed the unsubdivided baseline at some decision prefix, and a competitor's baseline selection is pushed later. That breaks the prefix inequalities the bound requires. No Arc 03 control runs both the baseline and subdivided workloads and evaluates the prefix inequalities per dimension and per competitor, and the existing rotation and yield controls exercise the per-scope mechanism at issue, so none of them may be cited as evidence for it. In particular, no control yet exists in the first required form — a finite decision prefix during which none of the compared newly admitted demands releases, with scope-bookkeeping cost isolated by a non-binding bookkeeping dimension or by equal prefunding. That condition is narrower than prohibiting all releases: unrelated releases elsewhere in the workload are permitted, and only a compared demand releasing inside the prefix would confound the comparison. Because no such control exists, the bookkeeping cost of subdivision has never been separated from amplification in any Arc 03 result. The property is unchanged and remains basal; the shortfall is in the provider. The correction belongs to the named provider fairness slice, which must bind each pending demand to the fixed `FairnessRoot` that owns it rather than to a per-scope identity, and must supply the missing control. Until that control exists and passes, this obligation is reported as open and failing. Arc 03 claims no pass for it here.
...
Accepted CI at exact head `6a22911` establishes **runtime non-regression only**. It shows that the retained connector, lifecycle, cleanup, ICE-restart, codec-neutrality, compatibility, and TURN behavior still runs as accepted at that head. It is **not** proof of P6 partition non-amplification, not proof of any contraction behavior over `S`, `Gc`, `O`, `T`, or `B`, not proof of the hostile-ingress progress and backpressure obligation, not proof of substrate backing, and not a resolution of the unproved-backing question. None of those has a control at that head, so a passing run cannot have exercised them, and no part of that CI result may be cited toward them.
...
**Unproved backing is an explicit open Slice C residual.** No Arc 03 control establishes `B`, so no dimension has proved enforceable substrate backing at this head, and Arc 03 asserts none. Every dimension is therefore an unproved-backing residual, and the Slice C residual-enforcement question — how a named residual becomes an enforced charge, whether by adapter hook, external isolation boundary, conservative over-charge, or disclosed non-enforcement — remains open and is not decided here. No control, measurement, or CI result in this document may be cited as closing it.

Arc 03 does not claim complete hostile-ingress admission, exact native dependency memory accounting, Endpoint Auth verification, authenticated session authority, final application flow authority, final codec policy, repository-wide close fencing, enforceable substrate backing, Slice C closure, or LegacyV1 removal.

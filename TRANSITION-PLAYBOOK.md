...
**P6 is finite-trace non-amplification.** Fix FairnessRoot A and a competing FairnessRoot B. Fix one finite trace. Vary only how A's demand in that trace is partitioned across AttributionChildScopes beneath A. Against the unpartitioned baseline, no such repartition may give A **earlier eligibility**, **an additional selection or turn**, or **a greater admitted quantity**, and none may **delay B**. Those four observables are the whole property, it is checkable on a bounded execution, and it needs no limit argument.

**It is one-way, not equality.** P6 does not require total outcome or share equality and does not require an identical replay. A repartition that leaves A worse off, or leaves B better off, is fully conforming: the property is a ceiling on what subdivision can gain, not a guarantee that subdivision is free. Do not state it as "the partition is invisible" — that is too strong and would fail correct providers.

It fixes no share, no ratio, no quantum, and no scheduler. A provider may give roots wildly unequal outcomes for any local reason and still satisfy P6. Conformance tests compare exactly the four observables above under repartition; a test that only shows each scope is eventually served is a different and weaker statement and does not test P6.
...
Scheduling outcome is a property of FairnessRoots. Accounting detail is a property of AttributionChildScopes. A provider that rotates over AttributionChildScope identities instead of FairnessRoots makes a root's replay depend on how its demand is subdivided beneath it, which is exactly the non-amplification above failing.
...
8. Test the property gate and the provider policy separately. Property tests cover identity rotation, many-source pressure, unequal claim sizes, exact release, conservation under concurrent charges, child-scope borrowing, cooperative retirement, ignored retirement, failed-cleanup retention, pre-reserved cleanup under a full grant, optional ceilings including the all-absent case, slow work, and storage-backed work. Provider-policy tests cover move-only pending demand, exact-charge reservation, cooperative entry, drop-cancellation, authority ordering, equal-class rotation, victim-set proof, and arbitration determinism against the installed provider. Add the P6 test as a finite-trace non-amplification test: fix two FairnessRoots and one trace, replay it with the first root's demand partitioned differently across AttributionChildScopes beneath it, and assert against the unpartitioned baseline that the repartitioned root gains no earlier eligibility, no additional selection or turn, and no greater admitted quantity, and that the competing root is not delayed. Compare exactly those four observables. Do not assert total outcome or share equality, and do not fail a repartition that leaves the repartitioned root worse off or the competing root better off. Record the test as failing rather than omitting it. A test that only shows each scope is eventually served does not test P6. Property tests must also cover P4-constrained refusal (a fitting claim refused while usable capacity sits idle and unreserved is a failure, outside the three exception classes) and safe committed-grant contraction (drive `H` below `S` under live leases and assert `Gc >= S` throughout, that no lease is shrunk, revoked, or forcibly released, that no admission draws on the `H`-to-`S` shortfall, that the degraded state is reported, and that `Gc` only follows `H` downward after releases lower `S`). The property gate additionally requires the P6 non-amplification control described above, which no named control currently implements. That control is outstanding and blocking for Slice D; the property gate is not satisfied merely because the provider-policy tests pass.
...
- **Connector-local scheduling metadata is not capacity.** A scheduling weight, quantum, or share is connector-local ordering only; it reserves no provider capacity, guarantees no cross-scope admission, and multiplying it multiplies no grant. This is a claim about capacity authority and does not discharge P6 partition invariance below.
- **Partition invariance of scheduling (P6).** Over one fixed finite trace with two fixed FairnessRoots, repartitioning one root's demand across AttributionChildScopes beneath it gives that root no earlier eligibility, no additional selection or turn, and no greater admitted quantity, and delays the competing root not at all. The gate control compares exactly those four observables against the unpartitioned baseline; it must not require total outcome or share equality, and must not substitute eventual service of each scope. P6 claims nothing about real-world claimants and gives no Sybil resistance; those are unsupported claims rather than pending work. **Not satisfied at this disposition.** The installed deterministic finite provider rotates equal-authority turns over AttributionChildScope identities rather than over FairnessRoots, so the rotation cursor advances per scope key and repartitioning a fixed root's demand beneath it changes that root's replay — additional selections and turns for that root, and corresponding delay to the competing root. The failure is in the rotation key, not in any quantity the provider computes. No named control proves this obligation. It is a blocking Slice D obligation under review 4865297956 §8 and must not be reported as passing.
...
```
  ...
  `H` is an input to the owner of `Gc`; an instantaneous `H` is never assigned to `Gc`. The invariants: `Gc >= S` always, and contraction never sets `Gc` below the charge already committed; `H < S` is a typed degraded or over-committed state *relative to `H`*, not a P1 violation, because P1 is stated against `Gc` and `Gc >= S` still holds; no admission draws on the shortfall between `H` and `S`; retirement requests go only to the exact owner of a lease its own contract declares reclaimable, sticky and without a timer; all unreleased charges stay retained, with owner Drop after cleanup or explicit failed-cleanup retention the only path out of a charge (P2); and `Gc` contracts toward `H` only after releases lower `S`, never below the prevailing `S`. Contraction is an outcome of releases, never a reclamation mechanism, and the degraded interval is disclosed rather than smoothed.
...
- arbitration selects `Cleanup > Admitted > Speculative`, with equal-class rotation across process-local AttributionChildScope identities. Because the rotation cursor advances per scope key rather than per FairnessRoot, repartitioning a fixed root's demand beneath it changes that root's replay, which fails P6 non-amplification in the property gate above. The provider is P6-nonconforming until Slice D keys the rotation to a FairnessRoot selected by the trusted provider or ingress owner;
...
- the contraction policy: what supplies `H`, how often it is sampled, and how far `Gc` is allowed to trail it;

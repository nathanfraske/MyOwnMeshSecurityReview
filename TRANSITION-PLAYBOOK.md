...
fixed input workload
    ...
    per demand: exact claim, authority class, reclaimability
...
The baseline run is that workload with root `A`'s demand unsubdivided. The subdivided run is the same workload with `A`'s demand spread across additional AttributionChildScopes beneath `A`, preserving the demands, their order, their claims, their authority, and their reclaimability, and changing nothing else.
...
- **Partition non-amplification, or subdivision monotonicity (P6).** Over one fixed *input workload* with releases derived, subdividing root `A`'s demand across additional AttributionChildScopes beneath `A` must satisfy, **at every decision prefix `k`**: cumulative selections of `A` no greater than baseline, cumulative admitted quantity of `A` no greater than baseline **in every resource dimension `d`**, and every baseline selection of every competitor `B != A` occurring no later than its baseline index, with an absent selection taking index infinity. The gate control compares exactly those quantities against the unsubdivided baseline; it must not require total outcome or share equality, and must not substitute eventual service of each scope. The **first** such control evaluates a finite decision prefix during which none of the compared newly admitted demands releases — unrelated releases elsewhere need not be prohibited — and isolates scope-bookkeeping cost either by charging bookkeeping to a dimension non-binding for the workload or by prefunding both runs equally, stating which method it used. P6 claims nothing about real-world claimants and gives no Sybil resistance; those are unsupported claims rather than pending work. **Not satisfied at this disposition.** The installed deterministic finite provider rotates equal-authority turns over AttributionChildScope identities rather than over FairnessRoots, so the rotation cursor advances per scope key: subdividing a fixed root's demand beneath it pushes that root's cumulative selections above the unsubdivided baseline at some decision prefix, and pushes at least one competitor's baseline selection to a later decision index. The failure is in the rotation key, not in any quantity the provider computes. No named control proves this obligation. It is a blocking Slice D obligation under review 4865297956 §8 and must not be reported as passing.
...
- **P4-constrained immediate refusal.** A provider may implement no pending retention and answer with immediate typed pressure instead — but **only when the claim does not currently fit.** A fitting claim must be admitted. Declining to implement retention decides how a non-fitting claim is answered, never whether a fitting one is admitted. P4 work conservation binds independently of retention: refusing a fitting claim while capacity able to satisfy it is neither live nor reserved for an in-flight admission is a P4 violation. Exactly three exception classes permit refusing a fitting claim, and no others are recognized: **(1)** a proven provider structural limit applies — proven, not assumed, since an unproven claimed limit is a stop condition; **(2)** an explicit local isolation policy or optional ceiling refuses it, and the refusal names that policy; **(3)** provider accounting is unavailable, poisoned, or cannot prove the admission safe, in which case refusing is correct and admitting on unprovable accounting is not. Fit itself is evaluated against the **committed and actually backed admission domain with P5 policy applied** — `Gc` net of `S`, additionally bounded by `B` wherever substrate backing is claimed, and further narrowed by any explicit isolation policy or optional ceiling — never against `Gc` alone and never against a transient `O`. Capacity reserved for an in-flight admission means the claim does not currently fit and is **not** a separate exception. Typed backing loss (`B < Gc` or `B < S`) is an affected-capacity, unprovable-safe-accounting, degraded-state condition covered by class 3 with the fit condition, and is **not** a separate class. A refuse-only provider is nonconforming, not trivially conforming. Nothing here implies liveness, a queue, or a deadline, and every refusal is a typed availability result under P7.
...
```text
S    committed charge = live claims + failed-cleanup-retained claims
Gc   provider-owned committed grant = what admission is checked against
...
B    enforceable substrate backing or assigned hard domain
         (cgroup, job object, process limit, appliance, provider allocation)
```

  Their authorities differ. **`O` alone changes nothing** — no grant, no admission result, no contraction state; only a named, recorded policy action may convert an observation into a new `T`, and **P4 fit is never computed against `O`**. **`T` is an owner decision with gradual follow**: `T < Gc` *requests* contraction, and `Gc` follows `T` downward only after owner-driven release lowers `S`, never below `S`. **`B` is the only quantity that enforces**: a provider claiming substrate-backed enforcement must prove `Gc <= B` at admission, and absent that proof it may not claim substrate backing at all. The invariants: `S <= Gc` continuously; `B < Gc` or `B < S` is a typed backing-loss or external-overcommit state that retains every unreleased charge, refuses conflicting new work in that dimension, and **does not pretend the missing physical backing exists**; retirement requests go only to the exact owner of a lease its own contract declares reclaimable, sticky and without a timer; owner Drop after cleanup or explicit failed-cleanup retention remains the only path out of a charge (P2). Contraction is an outcome of releases, never a reclamation mechanism. **If backing cannot be proved, that remains the open Slice C question** — the provider has not failed, it simply may not assert substrate-backed enforcement.
...
- arbitration selects `Cleanup > Admitted > Speculative`, with equal-class rotation across process-local AttributionChildScope identities. Because the rotation cursor advances per scope key rather than per FairnessRoot, subdividing a fixed root's demand beneath it breaks the prefix inequalities P6 requires in the property gate above. The provider is P6-nonconforming until Slice D keys the rotation to a FairnessRoot selected by the trusted provider or ingress owner;
...
- arbitration reads no clock, entropy, or host probe, and **the determinism claim is narrow**: it holds only over a fixed set of already-issued `ResourceScopeId`s, an identical starting provider state, and an identically ordered operation sequence, where replay yields identical admission outcomes. It is not a claim that two process runs agree — scope identifiers are allocated per run and rotation cursors key to them — not a claim about concurrent operations arriving in a different order, and not reproducibility of timing or of any measurement;
...
**A passing CI run proves only what it has controls for.** Accepted CI at exact head `6a22911` is runtime non-regression evidence only: retained runtime behavior still runs as accepted at that head. It does not prove P6 partition non-amplification, does not prove grant contraction over `S`, `Gc`, `O`, `T`, or `B`, does not prove hostile-ingress progress or backpressure, does not prove enforceable substrate backing, and does not close the Slice C residual-enforcement question. No control for any of those exists at that head, so the run cannot have exercised them, and no part of that result may be cited toward them. State this limit wherever that run is cited as evidence.
...
- the contraction policy: what supplies `O`, the named recorded policy action that converts an observation into a `T`, and what establishes `B` where substrate backing is claimed;

...
#[tokio::test]
async fn test_add_remote_candidate_return_proves_internal_insertion() -> Result<()> {
    ...
    let stats = agent.get_remote_candidates_stats().await;
    assert_eq!(stats.len(), 1, "the awaited insertion must be visible");
    assert_eq!(stats[0].id, candidate.id());
    assert_eq!(stats[0].ip, candidate.address());
...
#[tokio::test]
async fn test_disabled_mdns_candidate_is_not_reported_as_applied() -> Result<()> {
    ...
    let candidate: Arc<dyn Candidate + Send + Sync> = Arc::new(
        CandidateHostConfig {
            base_config: CandidateBaseConfig {
                network: "udp".to_owned(),
                address: "unresolved-test-candidate.local".to_owned(),
                port: 5000,
                component: 1,
                ..Default::default()
            },
    ...
    let result = agent.add_remote_candidate(&candidate).await;
    assert_eq!(result, Err(Error::ErrCandidateIpNotFound));
    assert!(agent.get_remote_candidates_stats().await.is_empty());

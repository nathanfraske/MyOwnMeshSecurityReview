...
#[tokio::test]
async fn test_add_ice_candidate_propagates_disabled_mdns_rejection() -> Result<()> {
    ...
    // This white-box seed isolates the public add_ice_candidate boundary from
    // SDP negotiation. The method only requires an established remote
    // description before forwarding the candidate to the ICE transport.
    *peer_connection
        .internal
        .current_remote_description
        .lock()
        .await = Some(RTCSessionDescription::default());

    let result = peer_connection
        .add_ice_candidate(RTCIceCandidateInit {
            candidate: "candidate:1380287402 1 udp 2130706431 unresolved-test-candidate.local 60542 typ host"
                .to_owned(),
    ...
    assert_eq!(
        result,
        Err(Error::Ice(ice::Error::ErrCandidateIpNotFound)),
        "RTCPeerConnection must propagate the ICE agent's disabled-mDNS rejection"
    );

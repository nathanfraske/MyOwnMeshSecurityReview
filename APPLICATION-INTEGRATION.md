...
- **Non-multipliable service share.** Scheduling attributes a demand to a FairnessRoot: a trusted local process or ingress scheduling attribution, assigned by the local process and never minted, chosen, or supplied by the claimant it attributes. A FairnessRoot is not a Device ID, Mesh Context, durable identity, endpoint identity, authentication or authorization root or capability, or any wire-visible value, and it carries no authority. Attribution child scopes refine accounting beneath exactly one FairnessRoot and cannot create a second root, share, or turn. **This property is not satisfied by the resource provider shipped today. See the disclosed gap below.**
...
Disclosed gap: that reference provider does not satisfy the non-multipliable service share property. Its equal-class rotation is keyed to a process-local scope identity that a claimant can create more of, rather than to a FairnessRoot, so one claimant that creates several attribution child scopes receives proportionally more turns than a claimant with one. An application must not rely on cross-claimant fairness from the current provider, and must not present it as a fairness boundary between tenants, accounts, or peers. The correction is an open obligation on the resource provider's fairness slice; the current algorithm, the exact mechanism, the evidence status, and the remediation destination are recorded in [`IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md`](IMPLEMENTATION-CONSTRAINTS-AND-INVARIANTS.md). This disclosure concerns service share only; it does not weaken the conservation, cleanup-ownership, honest-retention, pressure, or time properties above, and disclosing it does not discharge it.
...
- attribution child scopes cannot manufacture service share: creating, cloning, or rotating any number of attribution child scopes beneath one FairnessRoot does not improve that claimant's service share against another FairnessRoot — **this gate is NOT satisfied by the `FiniteResourceProvider` shipped today, which schedules against a mintable process-local scope identity. Application integration must be reported as failing this gate. No existing control proves it, disclosure of the gap does not satisfy it, and the gate must not be reworded into a per-scope guarantee the current provider already meets;**

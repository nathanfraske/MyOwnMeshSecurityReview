...
use crate::engine::peer_snapshot::PeerSnapshotStaging;
...
impl JoinedNetwork {
    ...
    /// Plan the same snapshot without building it.
    ///
    /// [`peers`](Self::peers) allocates the whole roster — including every
    /// peer's advertisement, whose size the peer chose — and only then hands it
    /// back. That is the right shape for a caller that owns the process. A
    /// caller answering somebody else's request needs to know the price first,
    /// and needs to be able to refuse it, so this returns a plan whose four
    /// claims are each separately fundable and separately refusable. See
    /// [`crate::PeerSnapshotStaging`] for the sequence and
    /// [`Mesh::local_application_resource_scope`] for where the leases come
    /// from.
    ///
    /// The published rows serialize byte for byte as `peers` does.
    pub fn plan_peers(&self) -> PeerSnapshotStaging<'_> {
        self.state.plan_peer_snapshot()
    }

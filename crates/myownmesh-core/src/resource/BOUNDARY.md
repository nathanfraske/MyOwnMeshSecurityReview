...
**Controls are landed in the working tree, and this document claims no execution result for them.** They run the two executions and compare them over an ordered trace of cooperative-admission dispositions on the non-failing path — acceptance, same-root refusal, immediate grant, arbitration grant, terminal pressure, and owner cancellation — complete within that scope and no wider, excluding fail-closed invariant cancellation and `release_scope` teardown cancellation. A superseded selection-only test was previously described as the formal control; that was an overclaim and was withdrawn. The comparison is built as Construction A — identical pre-existing roots, identical scope topology, identical bookkeeping charges, stable DemandIds, and only the DemandId-to-AttributionChildScope mapping differing — stepped by one deterministic clock-free reducer with terminal stuttering permitted, over a bounded decision prefix that begins from an identical state in both executions and in which none of the compared newly admitted demands releases. A negative fixture forces scope-keyed selection and requires the oracle to reject it, so the control is shown capable of failing rather than merely passing. Construction A isolates bookkeeping by holding the already-created topology and its already-charged claims identical before the prefix begins; a comparison that creates or charges different scope state in either run is not this control, and the normalization does not make scope bookkeeping free or unlimited — allocating a real scope record remains finite, charged, and fallible outside the comparison.

Four limits bound that result. It is **bounded**, never globally sufficient and never a whole-model proof over every P6 workload. It is **scoped to cooperative admission on the non-failing path** and no wider. It is **working-tree scope**, not exact-head and not hosted CI, with citation requiring exact-state local verification and exact-head hosted evidence recorded externally. Its **DemandIds are test-side and variant-specific** — carried as the provider-minted `demand_key` on `PendingAccepted`, `ArbitrationGrant`, `TerminalPressure`, and `Cancelled` where applicable, and bound by exact per-call decision-log index for `PendingRefused` and `ImmediateGrant` — and none is a caller-supplied provider identifier; this provider exposes no such identifier. And **no deployed multi-root mapping is claimed**: production has **one scheduling root**, beneath which **child subdivision cannot multiply turns**; **cross-root production fairness is not claimed**; **future trusted-root assignment belongs to its owning provider or ingress arc**; additional trusted-root minting and the cross-root exercises are `#[cfg(test)]` only; and a root is provider-private, with no public caller able to name, mint, or rebind one. A later generalization may carry a deterministic owner automaton instead of a bounded prefix.

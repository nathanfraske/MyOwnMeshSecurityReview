...
- work conservation constrains immediate refusal. An immediate, nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit is computed in two named steps, `AccountingFit` and then `EffectiveFit`, defined in the fit subsection below. Neither step reads a transient observation `O` or a contraction target `T`. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not be used to hold capacity for an anticipated demand, to smooth a demand rate, or to enforce an undeclared share; any such reservation is a partition and is conforming only as explicit local isolation policy;
...
**Fit: `AccountingFit`, then `EffectiveFit`.** Fit is evaluated per resource dimension, in two named steps.

```text
AccountingFit(d)
    the remaining committed accounting grant in dimension d, after the
    live and failed-cleanup-retained charge S(d), and after explicit P5
    isolation policy or optional local policy

EffectiveFit(d)
    AccountingFit(d)
        intersect E(d)   only where E is proved for d
        intersect B(d)   only where B is proved for d

    neither proved  ->  EffectiveFit(d) = AccountingFit(d)
    E proved only   ->  AccountingFit(d) intersect E(d)
    B proved only   ->  AccountingFit(d) intersect B(d)
    both proved     ->  AccountingFit(d) intersect E(d) intersect B(d)
```

The two intersections are independent and optional, per dimension. Proving one never requires or implies the other. Where neither is proved, `EffectiveFit` equals `AccountingFit` for that dimension; that equality is explicitly allowed and is the accounting-only case, not a defect and not a hidden claim. `O` and `T` never participate in either step: an observation is inert and a target is a request, and neither may narrow or widen fit.

A successful `AccountingFit` admission is an accounting commitment and nothing more. It does not guarantee allocator, kernel, runtime, transport, external-relay, or hardware success, and the allocation it authorizes may still fail after admission. A provider never presents unproved containment or backing as established: an accounting-only committed grant is explicitly an accounting commitment, not proof that substrate capacity exists or that allocation will succeed. Where `E` or `B` is proved for a dimension, admission additionally carries that premise, and neither proof, taken at admission, makes the premise hold later.
...
A provider claiming `B` in a dimension proves `Gc <= B` there at admission, and one claiming `E` proves `Gc <= E` there; each such proof requires the Slice C mapping described below. A provider proving neither claims nothing of the kind, and admission does not universally require either proof. If a proved `B` falls below `Gc` the provider reports typed loss of reserved capacity for that dimension; if it falls below `S` it reports typed external overcommitment. A proved `E` that is reduced beneath `Gc` or `S` is reported the same way against containment. In every such state each charge is retained, no release is forged or inferred, conflicting admission is refused with a typed result naming the dimension, and `Gc` is still not lowered below `S`. Such a proof taken at admission is historical: it does not make the premise hold later, and when it fails, substrate availability may genuinely have failed. Charges remain charged and commitments remain owed, but the underlying resource may simply not be there. These states are conservative accounting, not an assurance of physical backing, and must never be reported as one. Where an adapter can prove neither premise for a dimension, that shortfall is a named Slice C residual: it is not silently treated as contained or reserved and is counted into neither `E` nor `B`.

**Slice C mapping handoff.** Every `Gc <= E` or `Gc <= B` claim requires a mapping that Slice C must supply and this boundary does not implement. The mapping is dimension specific: it names the exact substrate quantity that is contained or reserved for that one resource dimension. It is unit correct: it states the conversion between the MyOwnMesh `ResourceClaim` quantity and that substrate quantity, in both directions of reading, with no implicit unit. And it is monotone: a larger charge in the dimension must correspond to no less of the mapped substrate quantity being contained or reserved, so that a proof at one charge level cannot be borrowed for a larger one. Where no such mapping exists for a dimension, that dimension stays accounting-only and remains an explicit named residual; it may not be reported as contained or reserved, and it is counted into neither `E` nor `B`. `OpaqueDependencyResidual` is the clearest case: absent a mapping that names one unit and proves the same dimension-specific, unit-correct, monotone relationship, it stays accounting-only and residual; being charged a number in that class never by itself makes it `E` or `B`. This paragraph is a textual handoff only. No mapping, adapter, or enforcement is implemented here.
...
The shipped `FiniteResourceProvider` proves neither `E` nor `B` in any dimension: its grant is an owner-supplied vector, and an owner-supplied vector is neither host containment nor host reservation. It is therefore accounting-only in every dimension, `EffectiveFit` equals `AccountingFit` throughout, and it proves `S <= Gc` and nothing more. Its numbers are honest bookkeeping, never an enforcement or availability claim, and no admission it makes is evidence that capacity exists or that an allocation will succeed. It also fixes its grant at construction, exposes no contraction entry point, models no `O`, and derives no `T`. It exercises none of the obligations in this subsection, no control exercises them, and the absence of a contraction path is not evidence that contraction is safe.

...
17. immediate refusal is constrained by work conservation: typed pressure replaces retention only for a claim that does not fit, and a fitting claim is admitted except under a proven structural limit, an explicit isolation policy or optional local ceiling, or accounting that is unavailable, unsafe, or poisoned. Fit is computed per dimension in two named steps: `AccountingFit`, the remaining committed accounting grant after the live and failed-cleanup-retained charge `S` and after explicit isolation or optional local policy; then `EffectiveFit`, which intersects `AccountingFit` with an enforceable containment envelope `E` only where `E` is proved and with reserved and owned capacity `B` only where `B` is proved, so that neither proved leaves `EffectiveFit` equal to `AccountingFit`. The two intersections are independent and optional per dimension and neither implies the other, and `O` and `T` never participate. A successful `AccountingFit` admission is an accounting commitment only and guarantees no allocator, kernel, runtime, transport, external-relay, or hardware success. Refusal never reserves capacity for an anticipated demand or enforces an undeclared share;
18. contraction keeps six per-dimension quantities distinct: `S` the committed charge, `Gc` the committed grant admission is checked against, `E` an enforceable containment ceiling that caps consumption and promises no availability, `B` capacity actually reserved for and owned by the provider, `T` an owner-selected target, and `O` an external observation. `E` and `B` are orthogonal premises; a provider may prove either, both, or neither per dimension, and one proving neither is accounting-only there. `S <= Gc` holds at every instant, so `S > Gc` is never exhibited or described and a grant contracts only down to the charges already committed. `O` is inert and authorizes nothing. `T` is owner-selected, either named directly or derived by a named, recorded policy that may consider an optional `O`, with no mandatory path from `O`, and by itself it lowers no grant, releases no charge, and refuses no admission. `Gc` follows `T` downward only after owner-driven release lowers `S`. A provider claiming `B` proves `Gc <= B` at admission and one claiming `E` proves `Gc <= E`, each under the Slice C mapping requirement; a failed premise is typed loss of reserved capacity or typed external overcommitment, and in both every charge is retained, no release is forged or inferred, conflicting admission is refused with a typed dimension-naming result, and `Gc` is still not lowered below `S`. Those states are conservative accounting, never an assurance that physical backing exists, and a typed report exists only while the process is alive and the condition is observable to it: process death emits no typed result and destroys the live in-process capabilities that would have carried one, recovery is an ordinary restart, and no claim is made that external reservations, provider-side allocations, or retained cleanup obligations outside the process necessarily vanish with it. Every `Gc <= E` or `Gc <= B` claim requires a Slice C mapping — dimension specific, unit correct between the MyOwnMesh `ResourceClaim` quantity and the substrate quantity contained or reserved, and monotone so a larger charge maps to no less substrate quantity — and a dimension with no such mapping stays accounting-only as a named Slice C residual; absent a mapping that names one unit and proves the same dimension-specific, unit-correct, monotone relationship, `OpaqueDependencyResidual` stays accounting-only and residual, and being charged a number in that class never by itself makes it `E` or `B`. The shipped provider proves neither premise, is accounting-only in every dimension, and proves `S <= Gc` and nothing more. No control in this catalog exercises contraction, `O`, `T`, `E`, or `B`, and none may be cited as if it did;

...
P6 control — implemented, with bounded evidence:
...
A Construction A control now implements that obligation against the live
provider, paired with a negative fixture that forces scope-keyed selection and
requires the oracle to reject it, so a passing result is not vacuous. The
shipped `FiniteResourceProvider` keys its rotation cursor and its reclaim cursor
to the `FairnessRoot`: a root is minted only for a parentless scope, every
ordinary child inherits it verbatim, and creating `AttributionChildScope` values
beneath one root introduces no additional rotation key. The disclosed
counterexample is removed.

Four limits bound that result and must be carried wherever it is cited. It is a
**bounded conformance result for the decisions it covers, never globally
sufficient and never a whole-model proof**. It is **working-tree evidence**, not
exact-head and not hosted CI. Its **DemandIds are test-side logical identifiers
mapped positionally from the provider's selection log**, not caller-supplied
provider identifiers. And **no deployed multi-root mapping is claimed** —
additional trusted-root minting and the cross-root controls are `#[cfg(test)]`
only, and production mints exactly one process root.

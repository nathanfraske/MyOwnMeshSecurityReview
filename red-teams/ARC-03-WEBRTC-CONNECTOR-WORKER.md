...
`AccountingFit` and `EffectiveFit` definitions, for the explicit accounting-only
...
- a nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit is computed per dimension in two named steps. `AccountingFit` is the remaining committed accounting grant in that dimension after the live and failed-cleanup-retained charge `S`, and after explicit P5 isolation or optional local policy. `EffectiveFit` is `AccountingFit` intersected with an enforceable containment envelope `E` only where `E` is proved for that dimension, and with reserved and owned capacity `B` only where `B` is proved: neither proved leaves `EffectiveFit` equal to `AccountingFit`, `E` proved alone gives `AccountingFit` intersect `E`, `B` proved alone gives `AccountingFit` intersect `B`, and both proved gives both intersections. The two are independent and optional per dimension, and neither implies the other. `O` and `T` never participate in either step. A successful `AccountingFit` admission is an accounting commitment only: it does not guarantee allocator, kernel, runtime, transport, external-relay, or hardware success, and a provider never presents unproved containment or backing as established. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not hold capacity for an anticipated demand, smooth a demand rate, or enforce an undeclared share, because any such reservation is a partition and is conforming only as an explicit local isolation policy;
- contraction is modeled over six distinct per-dimension quantities that may never be conflated: `S` the committed charge, `Gc` the provider-owned committed grant admission is checked against, `E` an enforceable containment ceiling that caps consumption and promises no availability, `B` capacity actually reserved for and owned by this provider, `T` an owner-selected contraction target, and `O` an external observation. `E` and `B` are orthogonal premises per dimension and are never substituted for each other; a provider proving neither in a dimension is accounting-only there. `S <= Gc` holds at every instant, so `S > Gc` is not a reachable state and no control may exhibit or describe one, and a grant contracts only down to the charges already committed. `O` is inert: it grants nothing, limits nothing, is never a fit test, and no refusal is justified by it. `T` is owner-selected — named directly by the owner, or derived by a named, recorded policy that may take an optional `O` into account — and there is no mandatory path from `O` to `T`; `T` by itself lowers no grant, releases no charge, and refuses no admission. `Gc` descends toward `T` only after owner-driven release lowers `S`, one safe step at a time. A provider claiming `B` proves `Gc <= B` at admission and one claiming `E` proves `Gc <= E`, each under the Slice C mapping below; a failed premise is reported as typed loss of reserved capacity or typed external overcommitment against the affected dimension, with every charge retained, no release forged or inferred, conflicting work refused with a typed dimension-naming result, and `Gc` still not lowered below `S`. Those states are conservative accounting, never an assurance that physical backing still exists; substrate availability may genuinely have failed. A typed report exists only while the process is alive and the condition is observable to it: process death emits no typed result and destroys the live in-process capabilities that would have carried one, recovery is an ordinary restart, and nothing here claims that external reservations, provider-side allocations, or retained cleanup obligations outside this process necessarily vanish with it. Every `Gc <= E` or `Gc <= B` claim requires a Slice C mapping that this record does not implement: dimension specific, naming the exact substrate quantity contained or reserved for that one dimension; unit correct, stating the conversion between the MyOwnMesh `ResourceClaim` quantity and that substrate quantity with no implicit unit; and monotone, so a larger charge corresponds to no less mapped substrate quantity and a proof at one charge level cannot be borrowed for a larger one. A dimension with no such mapping stays accounting-only and is a named Slice C residual, counted into neither `E` nor `B`; absent a mapping that names one unit and proves the same dimension-specific, unit-correct, monotone relationship, `OpaqueDependencyResidual` stays accounting-only and residual, and being charged a number in that class never by itself makes it `E` or `B`. **The shipped provider proves neither `E` nor `B` in any dimension — an owner-supplied vector is neither host containment nor host reservation, so it is accounting-only throughout, `EffectiveFit` equals `AccountingFit`, and it proves `S <= Gc` and nothing more — and it fixes its grant at construction, exposes no contraction entry point, models no `O`, and derives no `T`. No control below exercises any of them, and none may be cited as if it did;**

...
from `O`, and for the alive-and-observable limit on typed reports. A review
...
- a nonwaiting acquisition returns typed pressure instead of retaining a demand only for a claim that does not fit, meaning one that cannot be met from capacity that is neither live nor reserved for an in-flight admission, in every dimension the claim requires. Fit is computed against the committed domain with P5 policy applied and narrowed by whichever enforcement premises the provider actually substantiates in that dimension: `Gc` net of `S` always, additionally bounded by an enforceable containment ceiling `E` where containment is substantiated, additionally bounded by reserved and owned capacity `B` where reservation is substantiated, and narrowed by an explicit local isolation policy or optional local ceiling. `E` and `B` are orthogonal per-dimension premises; a provider may substantiate either, both, or neither, and neither implies the other. Fit never reads a contraction target `T`, and a transient observation `O` contributes nothing to fit under any premise. A fitting claim is admitted, except under a proven structural limit, under an explicit local isolation policy or optional local ceiling, or when the accounting needed to admit it safely is unavailable, unsafe, or poisoned. Refusal may not hold capacity for an anticipated demand, smooth a demand rate, or enforce an undeclared share, because any such reservation is a partition and is conforming only as an explicit local isolation policy;
- contraction is modeled over six distinct per-dimension quantities that may never be conflated: `S` the committed charge, `Gc` the provider-owned committed grant admission is checked against, `E` an enforceable containment ceiling that caps consumption and promises no availability, `B` capacity actually reserved for and owned by this provider, `T` an owner-selected contraction target, and `O` an external observation. `E` and `B` are orthogonal premises per dimension and are never substituted for each other; a provider substantiating neither in a dimension is accounting-only there. `S <= Gc` holds at every instant, so `S > Gc` is not a reachable state and no control may exhibit or describe one, and a grant contracts only down to the charges already committed. `O` is inert: it grants nothing, limits nothing, is never a fit test, and no refusal is justified by it. `T` is owner-selected — named directly by the owner, or derived by a named, recorded policy that may take an optional `O` into account — and there is no mandatory path from `O` to `T`; `T` by itself lowers no grant, releases no charge, and refuses no admission. `Gc` descends toward `T` only after owner-driven release lowers `S`, one safe step at a time. A provider substantiating `B` proves `Gc <= B` at admission and one substantiating `E` proves `Gc <= E`; a failed premise is reported as typed loss of reserved capacity or typed external overcommitment against the affected dimension, with every charge retained, no release forged or inferred, conflicting work refused with a typed dimension-naming result, and `Gc` still not lowered below `S`. Those states are conservative accounting, never an assurance that physical backing still exists; substrate availability may genuinely have failed. A typed report exists only while the process is alive and the condition is observable to it: process death emits no typed result and destroys the live in-process capabilities that would have carried one, recovery is an ordinary restart, and nothing here claims that external reservations, provider-side allocations, or retained cleanup obligations outside this process necessarily vanish with it. A dimension for which neither premise can be substantiated is a named Slice C residual and is counted into neither `E` nor `B`. **The shipped provider substantiates neither `E` nor `B` in any dimension — an owner-supplied vector is neither host containment nor host reservation, so it is accounting-only — and it fixes its grant at construction, exposes no contraction entry point, models no `O`, and derives no `T`. No control below exercises any of them, and none may be cited as if it did;**
